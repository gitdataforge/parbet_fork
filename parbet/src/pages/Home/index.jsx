...
import { MapPin, Calendar, Heart, RefreshCw, AlertCircle, ChevronDown, Navigation as NavigationIcon, Clock, CheckCircle } from 'lucide-react';
...
export default function Home() {
    ...
    // --- HIGH END CAROUSEL DATA (Matches specific requested banners) ---
    // Content is mapped to real search queries to drive API results
    const heroSlides = [
        {
            id: "world-cup-banner",
            title: "World Cup",
    ...
    const groupEventsByPerformer = (events) => {
        ...
        return Object.values(groups).map(g => {
            ...
            const formatLongDate = (dStr) => {
                const d = new Date(dStr);
                return `${d.toLocaleDateString('en-US', {weekday: 'short'})}, ${d.getDate()} ${d.toLocaleDateString('en-US', {month: 'short'})} • ${d.toLocaleTimeString('en-US', {hour: '2-digit', minute:'2-digit', hour12: false})}`;
            };

            let dateStr = '';
            if (g.events.length === 1) {
                dateStr = formatLongDate(first.commence_time);
            } else {
                dateStr = `${formatShortDate(first.commence_time)} - ${formatShortDate(last.commence_time)}`;
            }
    ...
    return (
        <div className="animate-fade-in w-full pb-20 overflow-x-hidden pt-2">
            ...
            {/* 1. TOP FILTERS (Location & Dates repositioned strictly ABOVE Carousel) */}
            <div className="flex items-center space-x-2 md:space-x-3 mb-6 px-4 overflow-visible relative">
                {/* Green Location Navigation Pill */}
                ...
                <div className="relative">
                    <button 
                        ...
                        className="bg-white border border-gray-200 text-brand-text px-4 py-2 h-10 rounded-[10px] text-sm font-medium flex items-center justify-center whitespace-nowrap shadow-sm hover:bg-gray-50 transition-colors"
                    >
                        <MapPin size={16} className="mr-2 text-gray-400"/> 
                        {userCity !== 'Loading...' ? `${userCity}, ${userCountry || 'IN'}` : 'Detecting...'} 
            ...
            {/* 2. ENLARGED HERO CAROUSEL (Matching image_99de87.jpg) */}
            <div className="relative w-full h-[240px] md:h-[350px] lg:h-[400px] rounded-[16px] overflow-hidden mb-6 group bg-gray-100 shadow-sm border border-gray-200 px-0 mx-0">
                <AnimatePresence mode="wait">
                    ...
                    >
                        <div 
                            className="relative w-[60%] md:w-[45%] h-full z-20 flex flex-col justify-end pb-12 px-6 md:px-16"
                            style={{ 
                                backgroundColor: heroSlides[currentHeroIndex].bgLeft,
                                clipPath: 'polygon(0 0, 100% 0, 85% 100%, 0 100%)' 
                            }}
                        >
                            <h2 className="text-4xl md:text-6xl lg:text-7xl font-black text-white mb-6 leading-none tracking-tight">
                                {heroSlides[currentHeroIndex].title}
                            </h2>
                            <button 
                                onClick={() => navigate(`/performer/${encodeURIComponent(heroSlides[currentHeroIndex].query)}`)}
                                className="border border-white/40 text-white hover:bg-white/10 w-max px-8 py-2.5 rounded-[8px] text-sm font-bold transition-colors"
                            >
                                See Tickets
                            </button>
                        </div>
                        ...
                        <div 
                            className="absolute top-0 bottom-0 right-0 w-[60%] md:w-[70%] z-10"
                ...
                {/* Heart/Favourite in Carousel */}
                <button 
                    onClick={() => handleRestrictedAction(`Favourite ${heroSlides[currentHeroIndex].title}`)}
                    className="absolute top-5 right-5 w-[36px] h-[36px] bg-black/60 rounded-full flex items-center justify-center hover:bg-black transition-colors backdrop-blur-sm z-30"
                >
                    <Heart size={16} className="text-white"/>
                </button>
            </div>

            {/* 3. PAGINATION DOTS (Centered below carousel) */}
            <div className="flex justify-center items-center space-x-3 mb-8">
                {heroSlides.map((_, idx) => (
                    <button 
                        key={idx} 
                        onClick={() => setCurrentHeroIndex(idx)}
                        className={`transition-all duration-300 rounded-full ${idx === currentHeroIndex ? 'bg-[#1a1a1a] w-3 h-3' : 'bg-gray-300 w-2.5 h-2.5'}`}
                    />
                ))}
            </div>
            ...
            <div className="mx-4 mb-14">
                <div className="w-full bg-black rounded-[14px] p-6 md:p-8 flex flex-col md:flex-row justify-between items-center text-white cursor-pointer shadow-lg hover:bg-gray-900 transition-colors">
                    <div className="flex items-center w-full md:w-auto justify-center md:justify-start mb-6 md:mb-0 space-x-6">
                        <div className="flex items-center space-x-3">
                            <svg viewBox="0 0 24 24" width="40" height="40" fill="#1DB954">
                                <path d="M12 0C5.4 0 0 5.4 0 12s5.4 12 12 12 12-5.4 12-12S18.66 0 12 0zm5.521 17.34c-.24.359-.66.48-1.021.24-2.82-1.74-6.36-2.101-10.561-1.141-.418.122-.779-.179-.899-.539-.12-.421.18-.78.54-.9 4.56-1.021 8.52-.6 11.64 1.32.42.18.479.659.24 1.02zm1.44-3.3c-.301.42-.841.6-1.262.3-3.239-1.98-8.159-2.58-11.939-1.38-.479.12-1.02-.12-1.14-.6-.12-.48.12-1.021.6-1.141C9.6 9.9 15 10.561 18.72 12.84c.36.18.54.84.24 1.2zm.12-3.36C15.24 8.4 8.82 8.16 5.16 9.301c-.6.179-1.2-.181-1.38-.721-.18-.6.18-1.2.72-1.38 4.26-1.26 11.28-1.02 15.721 1.621.539.3.719 1.02.419 1.56-.239.54-.959.72-1.56.3z"/>
                            </svg>
                    ...
                    <button className="bg-[#1DB954] text-black font-bold px-8 py-3 rounded-[30px] text-[15px] hover:bg-[#1ed760] transition-colors w-full md:w-auto shadow-sm">
                        Connect Spotify
                    </button>
                </div>
            </div>
            ...
            {(isLoadingMatches || apiError) && (
                <div className="mx-4 mb-8 p-4 bg-gray-50 rounded-[12px] flex items-center justify-between border border-gray-200">
                    <div className="flex items-center text-sm font-bold text-gray-700">
                        {isLoadingMatches ? (
                            <><RefreshCw size={16} className="animate-spin mr-2"/> Syncing verified events in {userCity}...</>
                        ) : (
                            <><AlertCircle size={16} className="text-red-500 mr-2"/> {apiError}</>
                        )}
                    </div>
            ...
            <div className="mx-4 w-auto border border-gray-200 rounded-[12px] p-6 mb-12 flex flex-col md:flex-row justify-between items-center bg-white cursor-pointer hover:shadow-md transition-shadow">
                ...
                <button className="bg-[#1c242b] text-white font-bold px-10 py-3.5 rounded-[10px] text-[15px] hover:bg-black transition-colors w-full md:w-auto shadow-sm mt-8 md:mt-0">
                    Subscribe
                </button>
            </div>
            ...
            {/* STRICT NEWSLETTER SUBSCRIPTION BLOCK */}
            <div className="w-full mt-24 mb-16 flex flex-col items-center justify-center text-center px-4">
                ...
                <div className="flex flex-col sm:flex-row items-end justify-center w-full max-w-[550px] mx-auto mb-6 space-y-6 sm:space-y-0 sm:space-x-6">
                    <div className="w-full sm:w-[350px] relative">
                        <input 
                            ...
                            className="w-full border-b border-gray-300 pb-3 px-1 focus:outline-none focus:border-[#4a7228] text-[#1a1a1a] placeholder-gray-500 text-[16px] transition-colors bg-transparent rounded-none" 
                        />
                    </div>
                    <button className="border border-[#4a7228] text-[#4a7228] px-6 py-3 rounded-[8px] font-bold hover:bg-[#F2F8ED] transition-colors whitespace-nowrap w-full sm:w-auto text-[15px]">
                        Join the List
                    </button>
                </div>
